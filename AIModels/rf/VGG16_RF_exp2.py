...
# SVM model: Training
print('RF training...')
model_random_forest = RandomForestClassifier(n_estimators=500).fit(train_subset, ytrain)
...
for raster_path in test_subset_path:
    ...
    # Classifying
    y_pred = model_random_forest.predict(features)
...
with open(path_report+'RF_Overall_results.text', 'w', encoding='utf-8') as f:
    f.write('Overall VGG16 + Random Forest performance report \n')
    f.write('1. F1-Score: \n')
    f.write(str(f1score))
    f.write('\n2. Accuracy:\n')
    f.write(str(acc))
